...
const formatTestsForTeacherAndSubject = (
...
export const getTestForTeacherAndSubject = async (
  subjectId: string,
  teacherId: string
): Promise<any | []> => {
  ...
  if (tests) {
    const formattedTests = formatTestsForTeacherAndSubject(tests)

    return reduceScores(
      groupBy(formattedTests, formattedTest => formattedTest.studentId)
    )
  }
